...
async function getInstagramProfile(instagramUsername: string): Promise<InstagramProfile | null> {
...
export async function scrapeRuns() {
  try {
    ...
    console.log(`Found ${data.totalEvents} events from ${data.totalClubs} clubs`);
        ...
        for (const event of clubData.events) {
          try {
            ...
            if (!eventName || !datetime || !clubData.clubName) {
              console.log(`Skipping event - missing required fields:`, { eventName, datetime, clubName: clubData.clubName });
              skippedRuns++;
              continue;
            }
            ...
            const existingRun = existingRuns.find(
              (r: Run) =>
                ...
                new Date(r.datetime!).getFullYear() === dateObject.getFullYear()
            );
