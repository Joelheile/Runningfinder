import { pgTable, foreignKey, text, timestamp, index, unique, integer, numeric, boolean, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
(table) => {
	return {
		...
		clubsAvatarFileIdUnique: unique("clubs_avatar_file_id_unique").on(table.avatarFileId),
	}
});
...
(table) => {
	return {
		...
		authenticatorUserIdCredentialIdPk: primaryKey({ columns: [table.credentialId, table.userId], name: "authenticator_user_id_credential_id_pk"}),
		authenticatorCredentialIdUnique: unique("authenticator_credential_id_unique").on(table.credentialId),
	}
});
